...
int main()
{
     ...
     */
    // 方式一：将字符串声明成一个字符数组
    char s1[20] = "Hello, world! s1~";  // 等价于  char s1[] = "Hello, world!";
    printf("s1 = %s\n", s1);
    // 方式二：将字符串声明成一个指针
    ...
    /**
     * 1.两种声明方式有何区别？
     *   区别1：指针指向的字符串，在C语言内部被当作常量，不能修改字符串本身。修改了字符串的第一个字符。这种写法是错的，会导致难
     *        以预测的后果，执行时很可能会报错。
     *   区别2：第二个差异是，指针变量可以指向其它字符串。字符数组变量不能指向另一个字符串。
     *
    ...
    // 验证 指针变量可以指向其它字符串
        // 即使s6被const修饰了，该指针变量指向的内容也是可以被修改的
    ...
    printf("s8 = %s\n", s8);
    char s9[20];
    //s9 = "Hello, world! s9~"; // 错误，不能直接把字符串赋值给字符数组变量，会报错。原因是字符数组的变量名，跟所指向的数组是绑定的，不能指向另一个地址。
